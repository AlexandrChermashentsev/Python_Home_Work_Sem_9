...
def draw_grid(scr):
#Нарисовать линию поверхность/черный цвет/нач. точк/конеч точк/толщина
...
def draw_tic_tac_toe(scr, items):
    for i in range(3):
        for j in range(3):
            if items[i][j] == "0":
                '''Отрисовка круга: поверхность, цвет, центр, радиус, толщина линии'''
...
while Flag:
    for event in pygame.event.get(): # Проверка не закрыл ли игрок игру
        if event.type == pygame.QUIT:
            Flag = False
    
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            pos = pygame.mouse.get_pos()
            # '''проверка на клик мышы'''
        ...
        bot_win = get_win_check(field, "0")
        if player_win or bot_win: # Прверка победы каждой из сторон
            game_over = True
            if player_win:
